            ...
            <div class="Acc" style="padding-bottom: 20px;">
                ...
                <div>
                    ...
                    <div itemscope itemtype="http://schema.org/Product" class="DetailDescription">
                        ...
                        ?>
                        <?if(!$AccI):?>
                        <h1 itemprop="name" class="no_print" title="<?=$Prefix?><?=$Dev?> <?=$Mod?>"><?else:?><h2 itemprop="name" class="no_print h1"><?endif;?><?=$Prefix?><?=$Dev?> <?=$Mod?><?if(!$AccI):?></h1><?else:?></h2><?endif;?>
                        <div style="margin-bottom: 30px;margin-top:-25px">
							...
							<?else:?>
							<div itemprop="description"><?/*=$arItem["PROPERTIES"]["UNI_TOP_BLOCK"]["~VALUE"]["TEXT"]*/?><?=$arItem["DETAIL_TEXT"]?><br /><br />
                                    Аккумулятор <span itemprop="brand">CRAFTMANN</span> аналогичен штатной батареи питания (<a href="/tools/search/?q=<?=$arItem["DISPLAY_PROPERTIES"]["ORIGINAL_CODE"]["VALUE"]?>"><span itemprop="model"><?=$arItem["DISPLAY_PROPERTIES"]["ORIGINAL_CODE"]["VALUE"]?></span></a>) и полностью совместим с <?=$Dev?> <?=$Mod?>. В комплект поставки входят: <?=$arItem["DISPLAY_PROPERTIES"]['COMPLECT']["VALUE"]?>.<br/>
                                    <div style="display:none;">Аккумулятор Craftmann позаботится о стабильной и производительной работе Вашего <?=$Dev?> <?=$Mod?>. Емкость батареи составляет <?=$arItem["DISPLAY_PROPERTIES"]["CAPACITY"]["VALUE"]?> мАч, благодаря чему Вы сможете достаточно длительный срок не перезаряжать аккумулятор при соблюдении условия умеренного использования мобильного устройства. Внушительным плюсом аккумулятора является безопасность его использования. Этому способствует микросхема, которая контролирует уровень температуры батареи, а также не допускает ее перезаряда, что существенно продлит срок ее службы.
									</div>
                                </div>
                                <div>
...
function toggle_show(id) {
	document.getElementById(id).style.display = document.getElementById(id).style.display == 'none' ? 'block' : 'none';
						...
						<div onClick="toggle_show('bla-bla2')"><h3 style="cursor:pointer;">Список совместимых устройств</h3>
		<div id="bla-bla2" style="display: none;">
			<div>
<?
...
while($ar_group = $db_old_groups->Fetch())
{
 ...
 print_r("<br/>");
 $res_parent = CIBlockSection::GetByID($ar_group['IBLOCK_SECTION_ID']);
 if($ar_res_parent = $res_parent->GetNext())
 {
$my_var_ar_group_CODE = $ar_group['CODE'];
$my_var_ar_res_parentNAME_and_ar_groupNAME = $ar_res_parent['NAME'] ." ". $ar_group["NAME"];

echo <<<EOT
<a href="/accumulators/$my_var_ar_group_CODE/"> $my_var_ar_res_parentNAME_and_ar_groupNAME </a>
EOT;
 }
}
?>
...
<!--Список совместимых устройств-->
                        ...
                        <?if(strlen($arItem['PROPERTIES']['PRICE']['VALUE']) > 0):?>
                            ...
                            <div  itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                                <div id="DBuyArea" style="background-color:#dfefff; padding:12px 18px 15px 18px;margin-bottom:15px; position:relative;">
                                    ...
                                    <div id="DPrice" style="color: #000000 !important;"><span  itemprop="price" class="DAmmount"><?=$arItem['PROPERTIES']['PRICE']['VALUE']?></span> <span class="DCurrency" style="font-size: 20px; padding-top: 10px;"><?if(0){?><?}?><img src="/bitrix/templates/.default/components/individ/phone.list/.default/images/ruble.png" style="border:0px"><span itemprop="priceCurrency" class="ruble" style="display:none;">RUB</span></span><div class="DSub">Цена производителя</div></div>
                                    <div class="clear"></div>
	...
	return true;">Подробно</a></div><div class="clear"></div></div>
                        ...
                        } else
                        {
                            ...
                            $imgtitle = 'Аккумулятор '.strtolower($arItem['NAME']);
                        }
                        ?>
...
<?
		 ...
		 if ($My_arSection = $My_rsSections->Fetch())
		 {
			if($My_arSection['PICTURE']!== NULL)
			{
				...
				echo '<img src="'.CFile::GetPath($My_arSection['PICTURE']).'" width="'.$My_file['width'].'" height="'.$My_file['height'].'" alt="'.$My_arSection['NAME'].'" title="'.$My_arSection['NAME'].'" class="PhonePic" style="max-height: 100px;max-width: 70px;"/>';
			}
                            ...
                            <table class="DProp">
                                ...
                                <?else://Свойства для остальных разделов?>
                                    ...
                                    <tr class="<?=$TrClass?>">
                                        ...
                                        <td class="DValueTD"><a href="/tools/search/?q=<?=$arItem["DISPLAY_PROPERTIES"]["ORIGINAL_CODE"]["VALUE"]?>"><?=$arItem["DISPLAY_PROPERTIES"]["ORIGINAL_CODE"]["VALUE"]?></a></td>
                                    </tr>
                                    <tr class="<?=$TrClass?>">
                                    ...
                                    <tr class="<?=$TrClass?>">
                                        ...
                                        <td class="DValueTD"><a href="/tools/search/?q=<?=$arItem["DISPLAY_PROPERTIES"]["ARTICLE"]["VALUE"]?>"><?=$arItem["DISPLAY_PROPERTIES"]["ARTICLE"]["VALUE"]?></a></td>
                                    </tr>
        ...
        ?>
            ...
            <div class="Discontinued" style="display: block">
                ...
                <div class="DetailDescription">
                    <h2 class="no_print"><?=$Prefix?><br/>  <span><?=$Dev?> <?=$Mod?></span> </h2>
